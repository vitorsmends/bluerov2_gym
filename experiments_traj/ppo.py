...
# ==========================================
# 2. GERADOR DE TRAJETÓRIA (Sincronizado)
# ==========================================
class TrajectoryGenerator:
    ...
    def get_reference(self, t):
        ...
        # Ajustado para rampa de 20s como no script de plotagem
        if t < 20.0:
...
# ==========================================
# 3. LOOP DE EXECUÇÃO REDUZIDO
# ==========================================
def run_trajectory_ppo():
    print("[INFO] Iniciando PPO Trajectory Tracking (Experimento Curto)...")
    ...
    # Reduzido de 2000 para 1000 passos (100 segundos)
    # Isso deixa o gráfico na mesma escala do PID e MPC
    for i in range(1000):
        t = i * dt
        ...
        curr_vel = np.array([obs["u"].item(), obs["v"].item(), obs["w"].item()])
        psi = obs["yaw"].item()
    ...
    env.close()
    print("Sucesso! Arquivo 'data_ppo_traj.csv' gerado para comparação.")
